...
public class PedidoControlador {
    ...
    public String guardarPedido(@ModelAttribute("pedido") Pedido pedido, BindingResult result, Model model, 
        ...
        for (int i = 0; i < itemIds.length; i++) {
            ...
            if (vino != null) {
                BigDecimal precioUnitario = precios[i]; // precio por botella
                BigDecimal precioCaja = precioUnitario.multiply(new BigDecimal(vino.getCantVinosxcaja()));
                int cantBotellas = cantidades[i];
                BigDecimal subtotal = precioUnitario.multiply(new BigDecimal(cantBotellas));

                PedidoDetalle detalle = new PedidoDetalle();
                detalle.setVino(vino);
                detalle.setPrecioUnitario(precioUnitario);
                detalle.setPrecioCaja(precioCaja);
                detalle.setCantidad(cantBotellas);
